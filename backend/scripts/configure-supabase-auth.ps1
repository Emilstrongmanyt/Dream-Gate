...
Write-Host "  6. Save"
Write-Host "  7. TestFlight build shows Sign in with Google on Log In / Create Account"
Write-Host ""
Write-Host "Sign in with Apple (native iOS checklist):"
Write-Host "  - Apple Developer App ID must have Sign in with Apple enabled"
Write-Host "  - Xcode entitlements are applied by AppleSignInPostProcess on CI builds"
Write-Host "  - Supabase Apple provider Client IDs: com.solodreams.dreamgate"
Write-Host "  - Optional: enable Allow users without email for private relay addresses"
Write-Host ""
Write-Host "Phone login (SMS OTP):"
Write-Host "  1. Open: $dashboardBase/providers"
Write-Host "  2. Phone provider -> Enable"
Write-Host "  3. Configure an SMS provider (Twilio, MessageBird, Vonage, or TextLocal)"
Write-Host "  4. Save"
Write-Host "  5. TestFlight build shows Log in with phone on Log In / Create Account"
Write-Host "  6. Players enter phone with country code, e.g. +15551234567"
Write-Host ""